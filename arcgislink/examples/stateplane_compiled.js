...
var m = google.maps, n, o, p, q = {$:null, R:false}, r = {}, s = {};
function t(a, b, c) {
  ...
  if(b instanceof L) {
    ...
    switch(e) {
      ...
      case "Transverse_Mercator":
        d.ha = parseFloat(t(c, '"Scale_Factor",', "]"));
        c = new P(d);
        break;
      case "Albers":
        d.N = parseFloat(t(c, '"Standard_Parallel_1",', "]"));
...
U.prototype.z = function(a) {
  var b = this;
  x(a, this);
  this.spatialReference = a.spatialReference.wkt ? s.aa(a.spatialReference.wkt) : r[a.spatialReference.wkid];
        ...
        a: {
          ...
          if(v) {
            for(var w = 0, ea = v.length;w < ea;w++) {
              ...
              if(u(h) && v[w].name.toLowerCase() === h.toLowerCase()) {
                h = v[w];
                break a
              }
            }
...
function X(a) {
  this.ga = a ? a.lods : null;
  this.v = a ? r[a.spatialReference.wkid || a.spatialReference.wkt] : p;
  if(!this.v) {
    throw new Error("unsupported Spatial Reference");
  }
  this.ba = a ? a.lods[0].resolution : 156543.033928;
  this.minZoom = Math.floor(Math.log(this.v.q() / this.ba / 256) / Math.LN2 + 0.5);
  this.maxZoom = a ? this.minZoom + this.ga.length - 1 : 20;
  if(m.Size) {
    this.ca = a ? new m.Size(a.cols, a.rows) : new m.Size(256, 256)
  }
  this.B = Math.pow(2, this.minZoom) * this.ba;
  this.Y = a ? a.origin.x : -2.0037508342787E7;
  this.Z = a ? a.origin.y : 2.0037508342787E7;
  if(a) {
...
X.prototype.fromLatLngToPoint = function(a, b) {
  ...
  d.x = (c[0] - this.Y) / this.B;
  d.y = (this.Z - c[1]) / this.B;
  return d
};
...
X.prototype.fromPointToLatLng = function(a) {
  ...
  a = this.v.n([a.x * this.B + this.Y, this.Z - a.y * this.B]);
...
function Z(a, b) {
  ...
  if(b.U) {
    var c = t(this.d.url, "", "://");
    this.da = c + "://" + b.U + t(this.d.url, c + "://" + t(this.d.url, "://", "/"), "");
    this.X = parseInt(t(b.U, "[", "]"), 10)
  }
...
Z.prototype.getTileUrl = function(a, b) {
  ...
  if(!isNaN(a.x) && !isNaN(a.y) && c >= 0 && a.x >= 0 && a.y >= 0) {
    var e = this.d.url;
    if(this.da) {
      e = this.da.replace("[" + this.X + "]", "" + (a.y + a.x) % this.X)
    }
    d = this.l || (this.W ? this.W.getProjection() : Y);
    if(!d instanceof X) {
      d = Y
    }
    var f = d.ca, g = 1 << b, i = new m.Point(a.x * f.width / g, (a.y + 1) * f.height / g);
...
new m.OverlayView;
var ja = U, ka = Z;var la = s.aa(2264, 'PROJCS["NAD_1983_StatePlane_North_Carolina_FIPS_3200_Feet",GEOGCS["GCS_North_American_1983",DATUM["D_North_American_1983",SPHEROID["GRS_1980",6378137.0,298.257222101]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]],PROJECTION["Lambert_Conformal_Conic"],PARAMETER["False_Easting",2000000.002616666],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",-79.0],PARAMETER["Standard_Parallel_1",34.33333333333334],PARAMETER["Standard_Parallel_2",36.16666666666666],PARAMETER["Latitude_Of_Origin",33.75],UNIT["Foot_US",0.3048006096012192]]');
window.onload = function() {
  var a = new ja("http://maps.ci.charlotte.nc.us/ArcGIS/rest/services/GET/BaseMap/MapServer");
  google.maps.event.addListenerOnce(a, "load", function() {
    try {
      var b = new ka(a), c = new $([b], {name:"StatePlane"}), d = {zoom:12, center:ha(a).getCenter(), mapTypeId:"stateplane", mapTypeControlOptions:{mapTypeIds:["stateplane", google.maps.MapTypeId.ROADMAP]}}, e = new google.maps.Map(document.getElementById("map_canvas"), d);
      e.mapTypes.set("stateplane", c);
      google.maps.event.addListener(e, "mousemove", function(g) {
        window.status = la.forward([g.latLng.lng(), g.latLng.lat()]).join(",")
      })
    }catch(f) {
      alert(f)
    }
